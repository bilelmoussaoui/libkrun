    ...
    thread::scope(|s| {
        ...
        display_worker.run(move |worker| {
            for (idx, display) in display_args.iter().enumerate() {
                let paintable = worker.create_paintable(idx, display.width as i32, display.height as i32);
                let picture = gtk::Picture::for_paintable(&paintable);
                let window = gtk::Window::builder()
                    .title(&format!("libkrun gui_vm - display {idx} ({}x{})", display.width, display.height))
                    .child(&picture)
                    .build();
                window.present();
