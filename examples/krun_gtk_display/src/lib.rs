...
pub struct DisplayBackendWorker {
    ...
    pub(crate) per_display_inputs: Vec<Vec<(PollableChannelSender<InputEvent>, DisplayInputOptions)>>,
    pub(crate) paintables: Vec<Option<ScanoutPaintable>>,
}

impl DisplayBackendWorker {
    pub fn create_paintable(&mut self, display_id: usize, width: i32, height: i32) -> gdk::Paintable {
    ...
    /// `on_activate` runs once after GTK is initialized — create paintables,
    /// windows, and attach input there.
